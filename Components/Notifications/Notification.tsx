    ...
    const rejectReq = async() => {
        if(mUid) {
            ...
            if (docSnap.exists()) {
                ...
                await setDoc(reqRef,{requests:newRequests},{merge:true}).then(()=> {
  ...
  return (
    ...
    !status && <Box sx={{color:`${label}`,width:"100%",display:"flex",alignItems:"center",height:"8vh",cursor:"pointer",borderRadius:"10px",}}>
        ...
        <Box sx={{display:'flex',maxWidth:"60%",width:"60%",justifyContent:"space-around"}}>
            ...
            <Button variant= 'outlined' color='error' sx={{width:"80px",borderRadius:"10px",fontSize:"10px"}} startIcon={<ClearIcon/>} onClick={rejectReq}>Reject</Button>
